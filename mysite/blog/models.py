...
class Comment(models.Model):
    """
        Модель комментария. Она содержит ForeignKey для привязки к определённой статье. Это отношение определено как
        "один ко многим", т.к. одна статья может иметь множество комментариев, но каждый комментарий (!) может быть
        оставлен только для одной статьи.

        Булево значение в поле 'active' добавлено для возможности скрыть некоторые комментарии, например в целях цензуры.
    """
